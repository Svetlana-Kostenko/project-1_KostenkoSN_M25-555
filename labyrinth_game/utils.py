...
def describe_current_room(game_state):
    ...
    if ROOMS[room]['items']:
        print("Предметы: ", end='')
        print(*ROOMS[room]['items'], sep=', ')
    print("Выходы: ", end='')
    print(*ROOMS[room]['exits'].keys(), sep=', ')  
...
def solve_puzzle(game_state):
    ...
    if ROOMS[room]['puzzle']:
        ...
        while cnt < 3:
            user_answer = input('--')
            cnt += 1
            if user_answer in ROOMS[room]['puzzle'][1]:
                print('Правильно!')
                ROOMS[room]['puzzle'] = None
                if AWARDS.get(room, None):                
                    print(f'Твоя награда: {AWARDS[room]}')
                    game_state['player_inventory'].append(AWARDS[room])
                break
            else:
...
def trigger_trap(game_state):
    ...
    if game_state["player_inventory"]:
        ind = pseudo_random(game_state["steps_taken"], len(game_state["player_inventory"]))
